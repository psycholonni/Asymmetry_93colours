...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/lonnigomes/thesis/Colour_Similartiy_90/Asymmetry_93colours.py',
...
#Create loop counter to switch positions of colour for each loop based on previous loop position
LoopCounter = 0 
...
centre_cross_blank = visual.ShapeStim(
    win=win, name='centre_cross_blank', vertices='cross',units='norm', 
    size=(0.03, 0.03),
    ori=0, pos=(0, 0),
    lineWidth=1.5, lineColor=[1,1,1], lineColorSpace='rgb',
    fillColor=[1,1,1], fillColorSpace='rgb',
...
for thisTrial_3 in trials_3:
    ...
    for thisTrial_2 in trials_2:
        ...
        # Location of stimulis in first loop
        if LoopCounter ==1:
            # Select stim position from array 
          Ecc = stimpositions[trialnumber];
          if Ecc == 'onetwo':
            circle1posx = -100
            circle1posy = 100;
          ...
          if Ecc == 'fourthree':
            circle1posx = 100
            circle1posy = -100;
         #Location of stimulus in second loop
        if LoopCounter == 2:
            # Select the stim position from array
        ...
        thisExp.addData("Ecc", Ecc);
        #thisExp.addData("circle1posx", circle1posx);
        #thisExp.addData("circle1posy", circle1posy);
        ...
        thisExp.addData("Ecc", Ecc);
        #thisExp.addData("circle2posx", circle2posx);
        #thisExp.addData("circle2posy", circle2posy);
        ...
        # update component parameters for each repeat
        # keep track of which components have finished
        blankscreenComponents = [centre_cross_blank]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if centre_cross_blank.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                centre_cross_blank.frameNStart = frameN  # exact frame index
                centre_cross_blank.tStart = t  # local t and not account for scr refresh
                centre_cross_blank.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(centre_cross_blank, 'tStartRefresh')  # time at next scr refresh
                centre_cross_blank.setAutoDraw(True)
            if centre_cross_blank.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > centre_cross_blank.tStartRefresh + 0.500-frameTolerance:
                    # keep track of stop time/frame for later
                    centre_cross_blank.tStop = t  # not accounting for scr refresh
                    centre_cross_blank.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(centre_cross_blank, 'tStopRefresh')  # time at next scr refresh
        ...
        for thisComponent in blankscreenComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        trials_2.addData('centre_cross_blank.started', centre_cross_blank.tStartRefresh)
        trials_2.addData('centre_cross_blank.stopped', centre_cross_blank.tStopRefresh)
